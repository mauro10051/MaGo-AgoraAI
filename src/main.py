...
def carica_progetto():
    ...
    if len(map1) > 0:
        var1m.set(1)  # Modifica lo stato del `Checkbutton`
    else:
        var1m.set(0)  # Cambia lo stato del `Checkbutton`
    check1m.config(state="disabled")

    check2m.config(state="normal")
    if len(valuto_mappa_max) > 0:
        var2m.set(1)  # Modifica lo stato del `Checkbutton`
    else:
        var2m.set(0)  # Cambia lo stato del `Checkbutton`
    check2m.config(state="disabled")

    check1s.config(state="normal")
    if len(map2) > 0:
        var1s.set(1)  # Modifica lo stato del `Checkbutton`
    else:
        var1s.set(0)  # Cambia lo stato del `Checkbutton`
    check1s.config(state="disabled")

    valuto_mappa_max = safe_read(project_dir / "valuto_mappa_max.txt")  

    check2s.config(state="normal")
    if len(valuto_mappa_sam) > 0:
        var2s.set(1)  # Modifica lo stato del `Checkbutton`
    else:
        var2s.set(0)  # Cambia lo stato del `Checkbutton`
    check2s.config(state="disabled")

    if len(dibattito)<100:
...
# Bottone progetto
save_button = ttk.Button(root, text="Load project",style="Gre.TButton", command=carica_progetto)
save_button.place(x=10, y=420, width=150, height=22)  # Imposta posizione e dimensione

# Bottone cancella progetto
save_button = ttk.Button(root, text="Delete project",style="Gre.TButton", command=delete_progetto)
save_button.place(x=400, y=420, width=100, height=22)  # Imposta posizione e dimensione



progetti = carica_progetti()
combo = ttk.Combobox(root, values=progetti, state="readonly",style="Gre.TButton")
combo.place(x=200, y=420, width=180, height=25)
